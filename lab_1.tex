...
I hope to learn more about include more of the functional
...
library, which extends on many of these capabilities, among others. I'm also a fan of local-variable type inference, which I believe should increase code readability.
\section*{Part B}
\subsection*{1 Poem}
...
And so are you!
\end{verbatim}
See \autoref{fig:two} for my short code example. It's also possible to accomplish this task with a single string using the newline character `\texttt{\textbackslash n}'.
\subsection*{2 Comments}
\textbf{1)} In this activity we are tasked with writing comments in our programs, which either start with `\texttt{//}' for single line comments, or are enclosed in `\texttt{/*}\ldots\texttt{*/}' for multiline comments. Multiline comments aren't, unfortunately, a feature of every language (for instance, \href{https://twitter.com/gvanrossum/status/112670605505077248}{Python}), so they are a convenience for designating blocks of text the compiler (or interpreter) should ignore completely. See \autoref{fig:three} for an example of using single-line comments in code.
...
\textbf{3)} Removing one of the comment's backslashes introduces a syntax error into our program, which is probably because Java's parser (see also \href{http://openjdk.java.net/groups/compiler/doc/compilation-overview/index.html}{\texttt{javac}'s compilation overview}) cannot `parse' the text into an AST. In other words, it doesn't know what to do with something like `\texttt{/ some text}' in the source file because the operation isn't defined in the language's grammar.
